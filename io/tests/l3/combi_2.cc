...
void setup() {
  ...
  for (int i = 0; i < 15; i++) {
    catalog.insert_record("ltable2", {std::to_string(i), i + 2});
  }
  for (int i = 5; i < 20; i++) {
...
int main() {
  auto test_name = std::string("combi_2");
  auto query = std::string("SELECT * FROM \
    (ltable1 LEFT OUTER JOIN rtable1 ON ltable1.l1b == rtable1.r1b) AS LOJ2, \
    (ltable2 FULL OUTER JOIN rtable2 ON ltable2.l2a == rtable2.r2a AND ltable2.l2b == rtable2.r2b) AS FOJ2 \
    WHERE LOJ2.r1b == FOJ2.l2b");
