...
// Idea:
// Same as left_1 but we incorporate a second criteria.
...
int main() {
  auto test_name = std::string("left_2");
  auto query = std::string(
      "SELECT * FROM (ltable LEFT OUTER JOIN rtable ON ltable.lb == rtable.rb AND ltable.la == rtable.ra) AS LOJ2"
  );
