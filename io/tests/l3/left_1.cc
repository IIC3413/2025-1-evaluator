...
// Idea:
// Simple left outer join. We aim to evaluate if submissions are able to
...
int main() {
  auto test_name = std::string("left_1");
  auto query = std::string("SELECT * FROM (ltable LEFT OUTER JOIN rtable ON ltable.lb == rtable.rb) AS LOJ1");
